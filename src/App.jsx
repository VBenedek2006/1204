      ...
      <header className="app-header">
        <div className="logos">
          ...
          <a href="https://www.kkszki.hu" target="_blank" rel="noopener noreferrer">
            <img src={schoolLogo} alt="Iskola logó" width="200" />
          </a>
        </div>
        <h1>Vite + React</h1>
      </header>
      ...
      <main>
        ...
        <section className="items-section">
                ...
                <ul>
                  {items.map((item) => (
                    <li key={item.id ?? item._id ?? JSON.stringify(item)}>
                      ...
                      <button onClick={() => fetchItemById(item.id ?? item._id ?? item)}>Részletek</button>
                    </li>
                  ))}
                </ul>
              ) : (
          ...
          <div className="item-details">
            ...
            {detailError && <p className="error">Hiba: {detailError}</p>}
            {selectedItem && (
